...
public partial class RegisterCSP : System.Web.UI.Page

{
    ...
    private void BindGrid()
    {
        ...
        using (var cmd = new SqlCommand(@"
            SELECT EAN, Status, Naam, Adres, Email, Datum, Redispatch, Capaciteitsbeperking
            FROM dbo.RegisterCSP
            ORDER BY Naam;", con))
    ...
    {
        gvCSP.EditIndex = e.NewEditIndex;
        BindGrid();
    }
    ...
    {
        // Key
        var oldEAN = gvCSP.DataKeys[e.RowIndex].Value.ToString();

        // Nieuwe waarden
        var row = gvCSP.Rows[e.RowIndex];
        string ean = gvCSP.DataKeys[e.RowIndex].Value.ToString();
        ...
        string redispatch = ((TextBox)row.FindControl("txtRedispatch_Edit")).Text.Trim();
        string capaciteitsbeperking = ((TextBox)row.FindControl("txtCapaciteitsbeperking_Edit")).Text.Trim();


        using (var con = new SqlConnection(ConnString))
        using (var cmd = new SqlCommand(@"
        UPDATE dbo.RegisterCSP
           SET Status =                 @Status,
               Naam   =                 @Naam,
               Adres  =                 @Adres,
               Email  =                 @Email,
               ...
               Capaciteitsbeperking =   @Capaciteitsbeperking
         WHERE EAN = @EAN;", con))
        {
            ...
            cmd.Parameters.AddWithValue("@Redispatch", (object)redispatch ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@Capaciteitsbeperking", (object)capaciteitsbeperking ?? DBNull.Value);
    ...
    protected void gv_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            GridViewRow footer = gvCSP.FooterRow;
            ...
            string redispatch = ((TextBox)footer.FindControl("txtRedispatch_New")).Text.Trim();
            string capaciteitsbeperking = ((TextBox)footer.FindControl("txtCapaciteitsbeperking_New")).Text.Trim();
            ...
            using (var con = new SqlConnection(ConnString))
            using (var cmd = new SqlCommand(@"
                INSERT INTO dbo.RegisterCSP (EAN, Status, Naam, Adres, Email, Datum, Redispatch, Capaciteitsbeperking)
                VALUES (@EAN, @Status, @Naam, @Adres, @Email, @Datum, @Redispatch, @Capaciteitsbeperking);", con))
            {
