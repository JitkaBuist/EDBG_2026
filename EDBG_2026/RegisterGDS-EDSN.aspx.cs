...
public partial class RegisterGDSEDSN : System.Web.UI.Page

{
    ...
    private void BindGrid()
    {
        ...
        using (var cmd = new SqlCommand(@"
            ...
            FROM dbo.RegisterGDSEDSN
            ORDER BY Naam;", con))
        using (var da = new SqlDataAdapter(cmd))
        {
            var dt = new DataTable();
            da.Fill(dt);
            gvGDSEDSN.DataSource = dt;
            gvGDSEDSN.DataBind();
        }
    }
    ...
    {
        gvGDSEDSN.EditIndex = e.NewEditIndex;
        BindGrid();
    }

    protected void gv_RowCancelingEdit(object sender, GridViewCancelEditEventArgs e)
    {
        gvGDSEDSN.EditIndex = -1;
        BindGrid();
    }
    ...
    {
        // Key
        var oldEAN = gvGDSEDSN.DataKeys[e.RowIndex].Value.ToString();

        // Nieuwe waarden
        var row = gvGDSEDSN.Rows[e.RowIndex];
        string ean = gvGDSEDSN.DataKeys[e.RowIndex].Value.ToString();
    ...
    protected void gv_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            GridViewRow footer = gvGDSEDSN.FooterRow;
            ...
            using (var con = new SqlConnection(ConnString))
            using (var cmd = new SqlCommand(@"
                INSERT INTO dbo.RegisterGDSEDSN (EAN, Status, Naam, Adres, Email, Datum)
