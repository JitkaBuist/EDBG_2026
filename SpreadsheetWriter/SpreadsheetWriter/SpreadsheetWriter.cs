...
namespace OpenXmlPowerTools.SpreadsheetWriter
{
    ...
    {
        public void ExportTableToExcel(DataTable dtGet, String Destination)
        {
            ...
            Workbook wb = new Workbook
            {
                Worksheets = new Worksheet[]
                {
                    new Worksheet
                    {
                        Name = dtGet.TableName,
                        TableName = dtGet.TableName,
                        ColumnHeadings =  ConvertToCollumn(dtGet),
                        Rows = ConvertToRow(dtGet)
                    }
                }
            };
            SpreadsheetWriter.Write(Destination, wb);
        }

        public void RFQExportQuote(DataTable dtInvoice, DataTable dtConnections, String Destination)
        {
            Workbook wb = new Workbook
            {
                Worksheets = new Worksheet[]
                {
                    new Worksheet
                    {
                        Name = dtInvoice.TableName,
                        TableName = dtInvoice.TableName,
                        //ColumnHeadings =  ConvertToCollumn(dtHeader),
                        Rows = QuoteConvertToRow(dtInvoice, dtConnections)
                    }
                }
            };
            SpreadsheetWriter.Write(Destination, wb);
        ...
        public IEnumerable<Cell> ConvertToCollumn(DataTable dataTable)
        {
        ...
        public IEnumerable<Row> ConvertToRow(DataTable dataTable)
        {
            ...
            foreach (DataRow drRow in dataTable.Rows)
            {
                var row = new Row
                {
                    Cells = ConvertToCell(dataTable, drRow)
                };
                rows.Add(row);
            }
            return rows.AsEnumerable();
        }
        public IEnumerable<Cell> ConvertToCell(DataTable dataTable, DataRow dataRow)
        {
            var cells = new List<Cell>();
            int intColNr = 0;
            foreach (System.Data.DataColumn column in dataTable.Columns)
            {
                //var cell = new Cell
                //{
                //    Value = GetFieldValue(column, dataRow, intColNr),//(dataRow[intColNr] == DBNull.Value) ? "" : dataRow[intColNr],
                //};
                Cell newcell = GetFieldValue(column, dataRow, intColNr);
                cells.Add(newcell);
                intColNr++;
            }
            return cells.AsEnumerable();
        }
        public Cell GetFieldValue(System.Data.DataColumn column, DataRow dataRow, int intColNr)
        {
            Cell cell = new Cell();
            switch (column.DataType.ToString())
            {
                case "System.Boolean": if (dataRow[intColNr] == DBNull.Value)
                    {
                        cell.CellDataType = CellDataType.Boolean;
                        cell.Value = false;
                    }
                    else
                    {
                        cell.CellDataType = CellDataType.Boolean;
                        cell.Value = (Boolean)dataRow[intColNr];
                    }
                    break;
                case "System.Byte": cell.Value = (dataRow[intColNr] == DBNull.Value) ? "" : dataRow[intColNr];
                    break;
                case "System.Char": cell.Value = (dataRow[intColNr] == DBNull.Value) ? "" : dataRow[intColNr];
                    break;
                case "System.DateTime": if (dataRow[intColNr] == DBNull.Value)
                    {
                        cell.Value = " ";
                    }
                    else
                    {
                        cell.CellDataType = CellDataType.Date;
                        cell.Value = ((DateTime)dataRow[intColNr]).ToString("dd-MM-yyyy");
                        //cell.Value = ((DateTime)dataRow[intColNr]).ToOADate();
                        cell.FormatCode = "dd-mm-yyyy";
                    }
                    break;
                case "System.Decimal": if (dataRow[intColNr] == DBNull.Value)
                    {
                        cell.Value = " ";
                    }
                    else
                    {
                        cell.CellDataType = CellDataType.Number;
                        cell.Value = (Decimal)dataRow[intColNr];
                        cell.FormatCode = "0.00";
                    }
                    break;
                case "System.Int16": if (dataRow[intColNr] == DBNull.Value)
                    {
                        cell.Value = " ";
                    }
                    else
                    ...
                    break;
                case "System.Int32": if (dataRow[intColNr] == DBNull.Value)
                    {
                        cell.Value = " ";
                    }
                    else
                    {
                        cell.CellDataType = CellDataType.Number;
                        cell.Value = (Int32)dataRow[intColNr];
                        cell.FormatCode = "0";
                    }
                    break;
                case "System.String": cell.Value = (dataRow[intColNr] == DBNull.Value) ? "" : dataRow[intColNr];
                    break;
                default:
                    cell.Value = (dataRow[intColNr] == DBNull.Value) ? "" : dataRow[intColNr];
                    break;
            }
            return cell;
        }

        public IEnumerable<Row> QuoteConvertToRow(DataTable InvoiceTable, DataTable ConnectionsTable)
        {

            var rows = new List<Row>();
            String strVolmacht = "Nee";
            if (InvoiceTable.Rows[0]["Volmacht"].ToString() == "1") { strVolmacht = "Ja"; }
            var row = new Row
            {
                Cells = new Cell[]
                                {
                ...
                Cells = new Cell[]
                                {
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = InvoiceTable.Rows[0]["Name1"].ToString(),
                                    },
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = "",
                                    ...
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = InvoiceTable.Rows[0]["Address1"].ToString(),
                                    },
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = InvoiceTable.Rows[0]["Housenr"].ToString(),
                                    },
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = InvoiceTable.Rows[0]["Suffix"].ToString(),
                                    },
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = InvoiceTable.Rows[0]["Zip"].ToString(),
                                    },
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = InvoiceTable.Rows[0]["City"].ToString(),
                                    },
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = InvoiceTable.Rows[0]["Kvk"].ToString(),
                                    },
                                    new Cell {
                                        CellDataType = CellDataType.String,
                                        Value = InvoiceTable.Rows[0]["IBAN"].ToString(),
                                    },
                                }
            };
            rows.Add(row);
            ...
            foreach (DataRow drRow in InvoiceTable.Rows)
            {

                String strType = drRow["Type_Tekst"].ToString();
                DateTime dtmStartDatum = (DateTime)drRow["StartDatum"];
                DateTime dtmEindDatum = (DateTime)drRow["EindDatum"];
                string strProduct = "Beurs";
                if (drRow["Product"].ToString() == "1") { strProduct = "Vast"; }
                ...
                foreach (DataRow drConnections in ConnectionsTable.Rows)
                {
                    if (drRow["Type"].ToString().Trim() == drConnections["Type"].ToString().Trim())
                    {
                        row = new Row
                        {
                            Cells = ConvertToCell(ConnectionsTable, drConnections)
                        };
                        rows.Add(row);
                    }
                }
    ...
    public static class SpreadsheetWriter
    {
        ...
        {
            WorkbookStylesPart stylesPart = sDoc.WorkbookPart.WorkbookStylesPart;
